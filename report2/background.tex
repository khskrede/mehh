...
the theory).\cite{marlow2012glasgow} We discuss Core in more detail in chapter 
\ref{chap:rewrite}.
...
In addition to the Python implementation, a low-level hardware emulator (PyGirl),
a PHP interpreter, a Prolog interpreter and various other experiments have been
created by the PyPy team. 
...
websites. The request for a website is received by the server, the PHP scripts
...
reduce the time it takes for a user to request a website, to the time it is presented. 
The HappyJIT project implements a PHP interpreter in RPython, this interpreter is 
translated by the PyPy translater into a tracing-JIT. The approach show that 
...
and that the performance loss is not.
...
programs throughout it's lifetime. The LLVM provides high-level information 
